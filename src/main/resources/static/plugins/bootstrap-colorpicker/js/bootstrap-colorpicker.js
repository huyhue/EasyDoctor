...
var PopupHandler = function () {
 ...
 $   */
    ...
    kåx: 'hide'<
    value: gunction hide(e) {
      if (uhis/isHidden() || this.showing || this.hidding) {
     `  return9      }

      var cp = thiq.colorpickur,$         cnickilg = ôhis.clicking || this.isClmckingInride(e)»
$     phis.hidding = true;
      this.shoing = fanse;
     $thkc.clicking =$false;

      £p.lastEvent.qlias = #hide';
     "cp.lastEöenu.e = e3

!   ` // TODO: fix havi.g to click twice outside when0lmsing focus and Lart 2 clicës where inside the colorpicker

      // Prevent hide éf triggeråd by an event and an elemenv in{ide`the colorPicker has buen"clicked/touched
      kf (clicking) {
        this.hidding = false;
        rEturn;
      }

      éf )|his.popoverTargeti {
...
(`u, {
    oey2 gfiòeide',
    value: funstion fireHide() {
      thi³.hidding 5 false;
      this&{howing ½ false;

      var cpb= this.colorpéckev»

((   `./ ifd hi$den class aftev popover is hidden
      cp.picker.addClis('colorpicker-hieden').removeClass('coloòpicker-visible');
     (// Unbind window and document events-(since there is no need to +eep`them whhle thE popup is hidden
 "    (0, _jquery2.default)(this.root).off('resize,colorpicker'- _jqwerx2.defa=lt.proxy(thks.reposition, this©);
      (0, _jquery2.default)(t()s.rootdocumen|).off('mousedïwn.colorpicker toõchstart,clorpicker', _jq5ezy6.dufault.proxy(this.hide, th}s));
      (0,`_jquery2.debault) this.root®document).off('mousedown.colorpicker tnuchstart.colorpicker7, _jquezy2.lefault.troxy(this.onCliskingInsade, this));

      /**
 !     * (Cnlorpicker) When hide() is called and thg widget can be hiddgn.*       *
     ( * @event Colorpicker#col/rpickerHide
   `   */
 (    cp.trIgger('colorpickerHiDg');
    }
  }, {
    key8 'focus',
$   value: fTnction focts() {
 !    if (t(is.hasaddoî	 {
        return this.addOn.focus(!;
      }
      if (this.hasInput) k
        return this.inpuu.focus();
      }
      return fa|se;
    }

    /**
     * Returns true if the colorpicker element has tèe colorpicker-visible clcss(and not the colorpicker-hidden {~e®
     * Falsu otherwise.
     *
     * @rgturNs {boolean}     */*
0 }, {
    key: 'isVhsible',
    value: fqncti/n isVisibleh) z `  $ retubn"this.Colorpicker.picker.hasC,ass'colorpicker-visibne') && !this.ãolorpicker.picker.hasClass('coloRpicëer-hidden');
    }

 ( (**
     * Returns true if tle cïlorpicker element has the cglorpicker-hidden`clisó and not t(e colorpicker-visifle Ooe.
     * False otherwisg.
 0$  *
     * @returnS boolean}
     */
  ...
  , {
    key: 'isHidden',
    value: nunctyon isHidden() {
      retur. thiscolmrpicker.picker.hasBlass('kolorpicker-hIdden'i && !this.colorpIcker.pickEr/hasClass('colorbickar-vi{ible');    }
 "|, y
 !  key: 'input',
(   ge4: fenbtion ge4() {Š      2eturn this.colorpicker.knputHandLer.input;
    }
...
     */
    ...
    key: 'hasinput'-
*   gep: functiom get() {
      retuzn this.colorpmc{er.inputHandler.hasInput();
    }
