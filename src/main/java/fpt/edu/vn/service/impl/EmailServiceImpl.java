...
public class EmailServiceImpl implements EmailService {
	...
	public EmailServiceImpl(JavaMailSender javaMailSender, SpringTemplateEngine templateEngine,
			@Value("${base.url}") String baseUrl, JwtTokenServiceImpl jwtTokenService,
...
//            helper.setText(templateName, true);
	...
	public void sendAppointmentRejectionRequested(Appointment appointment) {
		...
		context.setVariable("url", baseUrl + "/appointments/acceptRejection?token="
				+ jwtTokenService.generateAcceptRejectionToken(appointment));

		sendEmail(appointment.getDoctor().getEmail(), "Yêu cầu từ chối lịch khám", "appointmentRejectionRequested",
				context, null);
	}
	...
	public void sendAppointmentRejectionAccepted(Appointment appointment) {
		Context context = new Context();
		context.setVariable("appointment", appointment);
		sendEmail(appointment.getPatient().getEmail(), "Yêu cầu từ chối lịch khám được chấp nhận",
				"appointmentRejectionAccepted", context, null);
	}
	...
	public void sendAppointmentCanceledByPatient(Appointment appointment) {
		...
		sendEmail(appointment.getDoctor().getEmail(), "Lịch khám bệnh đã hủy bởi bệnh nhân", "appointmentCanceled",
				context, null);
	}
	...
	public void sendAppointmentCanceledByDoctor(Appointment appointment) {
		...
		sendEmail(appointment.getPatient().getEmail(), "Lịch khám bệnh đã hủy bởi bác sĩ", "appointmentCanceled",
				context, null);
	}
	...
	public void sendAppointmentFinished(Appointment appointment) {
		Context context = new Context();
		context.setVariable("appointment", appointment);
		context.setVariable("url", baseUrl + "/appointments/reject?token="
				+ jwtTokenService.generateAppointmentRejectionToken(appointment));
		sendEmail(appointment.getPatient().getEmail(), "Lịch khám bệnh đã kết thúc", "appointmentFinished", context,
				null);
	}
	...
	public void sendInvoice(Invoice invoice) {
		...
		try {
			...
			sendEmail(invoice.getAppointments().get(0).getPatient().getEmail(), "Hóa đơn khám bệnh",
					"appointmentInvoice", context, invoicePdf);
		} catch (Exception e) {
